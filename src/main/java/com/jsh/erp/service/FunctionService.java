...
public class FunctionService {
    ...
    public List<Map<String, Object>> getCurrentUserBtnList() throws Exception {
        List<Map<String, Object>> userBtnList = new ArrayList<>();
        User userInfo = userService.getCurrentUser();
        if (userInfo == null) {
            return userBtnList;
        }
        ...
        String funIds = funList.get(0).getValue();
        if (StringUtil.isEmpty(funIds)) {
            return userBtnList;
        }

        String btnStr = funList.get(0).getBtnStr();
        ...
        List<Function> functionList = getFunctionListByIds(funIds);
        if (functionList == null || functionList.size() == 0) {
            return userBtnList;
        }

        Map<Long, Long> funIdMap = getCurrentUserFunIdMap();
        for (Function function : functionList) {
            if (PermissionUtil.isDefaultManager(userInfo) || funIdMap == null || funIdMap.get(function.getId()) != null) {
                Map<String, Object> item = new HashMap<>();
                item.put("url", function.getUrl());
                item.put("btnStr", btnMap.get(function.getId()));
                userBtnList.add(item);
            }
        }
        return userBtnList;
    }
